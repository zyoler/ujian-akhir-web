const elemenOutput=document.querySelector('#output-latest');
    elemenOutput.innerHTML="";
        fetch('https://neophyte-proxy.herokuapp.com/https://api-filmapik.herokuapp.com/latest')
        .then(function(response){
            return response.json();
        }).then(function(json){
            for(let i=0;i<json.result.length;i++){
                const elemendiv=document.createElement('div');
                elemendiv.className='latest';
